 ...
 *
 * Webyarns version:  3.8.10-vertical
...
(function( root, factory ) {
	...
	var VERSION = '(reveal 3.8.0) 3.8.10-vertical';
	...
	function navigateRight() {
		...
		var  nextSlideIdx= Webyarns.getNextSlideIndexH(currentSlide)
		if (nextSlideIdx){
			slide( nextSlideIdx);
		} else {
			// Reverse for RTL
			if (config.rtl) {
				if ((isOverview() || previousFragment() === false) && availableRoutes().right) {
					slide(indexh - Webyarns.noOfHiddenRight(currentSlide) + 1, config.navigationMode === 'grid' ? indexv : undefined );
				}
			}
			// Normal navigation
